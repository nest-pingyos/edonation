...
$id = $_GET['id'];
$table = $_GET['table'];  // รับค่าตารางจาก URL
...
$valid_tables = ['donat', 'receipt_2568', 'receipt_2567', 'receipt_2566'];  // กำหนดชื่อของตารางที่อนุญาต
...
// สร้างคำสั่ง SQL
$inv_mst_query = "SELECT T1.id, T1.billPaymentRef2, T1.payerAccountName, T1.billPaymentRef1, T1.amount, T1.address, T1.province, T1.amphure, T1.district, T1.zip_code, T1.project_name, T1.project_number, T1.receiptDate, T1.fiscal_year, T1.receipt_no, T1.payby FROM $table T1 WHERE T1.id='" . $id . "'";
...
if ($count > 0) {
    ...
    <table>
        ...
        <tr>
            ...
            <td align="right"><b>เลขที่ใบเสร็จ/Receipt No. </b>' . htmlspecialchars($inv_mst_data_row['fiscal_year']) . '-' . htmlspecialchars($inv_mst_data_row['receipt_no']) . '</td>
        </tr>
        <tr>
		...
		<tr>
			<td colspan="2" style="text-align: center;"><b>(ผู้ช่วยศาสตราจารย์ ดร.สุภารัตน์ วังศรีคูณ)<br>คณบดีคณะพยาบาลศาสตร์</b></td>
		</tr>
        <tr>
            <td><b>เลขที่ใบเสร็จ : </b>' . htmlspecialchars($inv_mst_data_row['fiscal_year']) . '-' . htmlspecialchars($inv_mst_data_row['receipt_no']) . '</td>
		    <td align="right"><b>ลำดับเอกสาร : </b>69' . str_pad(htmlspecialchars($inv_mst_data_row['id']), 4, '0', STR_PAD_LEFT) . '</td>
        </tr>
    </table>
';
