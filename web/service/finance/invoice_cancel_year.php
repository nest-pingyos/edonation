...
$showall = $_GET['showall'];  // รับค่าตารางจาก URL

// ตรวจสอบว่าได้รับค่าตารางหรือไม่
if (empty($showall)) {
    echo "ไม่มีการระบุชื่อของตาราง";
    exit;
}

// ตรวจสอบค่าตารางให้เป็นค่าที่ต้องการเท่านั้น (หลีกเลี่ยง SQL Injection)
$showall = isset($_GET['showall']) ? $_GET['showall'] : '';
$id = isset($_GET['receipt_id']) ? $_GET['receipt_id'] : '';  // ใช้ค่าจาก receipt_id ใน URL

$showalls = ['donat', 'receipt_2567', 'receipt_2566'];
if (!in_array($showall, $showalls)) {
    echo "ตารางไม่ถูกต้อง";
    exit;
}

// ใช้ id จาก URL มาเทียบกับฟิลด์ id ในฐานข้อมูล
$inv_mst_query = "
SELECT T1.id, T1.billPaymentRef2, T1.payerAccountName, T1.billPaymentRef1, T1.amount,
       T1.address, T1.province, T1.amphure, T1.district, T1.zip_code,
       T1.project_name, T1.project_number, T1.receiptDate, T1.fiscal_year,
       T1.receipt_no, T1.payby 
FROM $showall T1 
WHERE T1.id='" . mysqli_real_escape_string($con, $id) . "'";

$inv_mst_results = mysqli_query($con, $inv_mst_query);

if ($inv_mst_results === false) {
    echo "เกิดข้อผิดพลาดในการ query: " . mysqli_error($con);
    exit;
}
...
if ($count > 0) {
    ...
    $receiptDate = $inv_mst_data_row['receiptDate'];
    ...
    // 
    // ลายเช็ตคณบดี
    $img = '../../list/TCPDF/signature_T.png';
    ...
    // ลายน้ำ
    $img = '../../list/TCPDF/pdf_recrip_cc.png';
    $cellWidth = 220;
    $imageWidth = 255;
    $x = $pdf->GetX() + ($cellWidth - $imageWidth) / 2;
    $y = $pdf->GetY() - 10;

    $pdf->Image($img, $x, $y, $imageWidth, 300, '', '', '', false, 300, '', false, false, 0, false, false, false);
    ...
    <table>
        ...
        <tr>
            <td><b>ชื่อ/Name : </b>' . htmlspecialchars($inv_mst_data_row['payerAccountName']) . '</td>
            <td align="right"><b>เลขที่ใบเสร็จ/Receipt No. </b>' . htmlspecialchars($inv_mst_data_row['fiscal_year']) . '-' . htmlspecialchars($inv_mst_data_row['project_number']) . '-' . htmlspecialchars($inv_mst_data_row['receipt_no']) . '</td>
        </tr>
        <tr>
            <td colspan="2"><b>ที่อยู่/Address :</b> ' . htmlspecialchars($inv_mst_data_row['address']) . '' . htmlspecialchars($inv_mst_data_row['district']) . ' ' . htmlspecialchars($inv_mst_data_row['amphure']) . ' ' . htmlspecialchars($inv_mst_data_row['province']) . ' ' . htmlspecialchars($inv_mst_data_row['zip_code']) . '</td>
        </tr>
        ...
        <tr>
            <td colspan="2">' . htmlspecialchars($inv_mst_data_row['project_name']) . '</td>
        </tr>
        <tr>
            <td align="right" colspan="2"><b>จำนวนเงิน/Amount : </b>' . number_format($inv_mst_data_row['amount'], 2) . ' บาท</td>
        </tr>
        <br>
        <tr>
        ...
        <tr>
            <td colspan="2"><b>รวมทั้งหมด : ' . number_format($inv_mst_data_row['amount'], 2) . ' บาท (' . Convert($inv_mst_data_row['amount']) . ')</b></td>
        </tr>
        <tr>
            <td colspan="2"><b>Total Amount Received : ' . number_format($inv_mst_data_row['amount'], 2) . ' Baht (' . convertToEnglish($inv_mst_data_row['amount']) . ')</b></td>
        </tr>
        <tr>
            <td><b>ชำระโดย/Pay by : ' . htmlspecialchars($inv_mst_data_row['payby']) . '</b></td>
        </tr>
        <tr>
        ...
        <tr>
            <td><b>จาก : </b>' . htmlspecialchars($inv_mst_data_row['payerAccountName']) . '</td>
        </tr>
        <tr>
            <td colspan="2"><b>วัตถุประสงค์ : </b>' . htmlspecialchars($inv_mst_data_row['project_name']) . '</td>
        </tr>
        ...
        <tr>
            <td><b>เลขที่ใบเสร็จ : </b>' . htmlspecialchars($inv_mst_data_row['fiscal_year']) . '-' . htmlspecialchars($inv_mst_data_row['project_number']) . '-' . htmlspecialchars($inv_mst_data_row['receipt_no']) . '</td>
		    <td align="right"><b>ลำดับเอกสาร : </b>67' . str_pad(htmlspecialchars($inv_mst_data_row['id']), 4, '0', STR_PAD_LEFT) . '</td>
        </tr>
    </table>
';
