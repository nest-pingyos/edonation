...
class BenefitsController {
    ...
    public function handle(string $method, ?string $id, ?string $action): array {
        switch ($method) {
            ...
            case 'PUT':
                AuthMiddleware::requireAdmin();
                if (!$id) return Response::error('VALIDATION_ERROR', 'กรุณาระบุ ID');
    ...
    private function show(string $id): array {
        $sql = "SELECT id, name, amount, img_file, description, sort_order, is_active,
                ...
                FROM benefits WHERE id = :id LIMIT 1";
    ...
    private function create(): array {
        ...
        $sql = "INSERT INTO benefits (name, amount, img_file, description, sort_order, is_active)
    ...
    private function update(string $id): array {
        ...
        $checkSql = "SELECT id FROM benefits WHERE id = :id";
        ...
        $sql = "UPDATE benefits SET " . implode(', ', $updates) . " WHERE id = :id";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute($params);
