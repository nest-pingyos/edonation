...
class ReceiptController {
    ...
    public function handle(string $method, ?string $id, ?string $action): array {
        ...
        if ($id && $action) {
            switch ($action) {
                case 'pdf': return $this->getPdf($id);
                case 'verify': return $this->verifyTaxId($id);
                case 'details': return $this->getDetails($id); // สำหรับ pdf_maker.php
                case 'cancel':
                    AuthMiddleware::requireAdmin();
    ...
    private function search(): array {
        ...
        if ($isIdCard) {
            ...
            $sql = "SELECT 
                        ...
                        bt.billPaymentRef2
                    FROM receipts r
                    LEFT JOIN donat_user du ON r.donation_id = du.id
    ...
    private function index(): array {
        ...
        // Count total
        $countStmt = $this->pdo->query("SELECT COUNT(*) FROM receipts");
    ...
    private function generate(): array {
        ...
        $countStmt = $this->pdo->prepare(
            "SELECT COUNT(*) FROM receipts WHERE receipt_no LIKE :prefix"
        );
    ...
    private function cancel(string $id): array {
        $checkStmt = $this->pdo->prepare("SELECT id FROM receipts WHERE id = :id");
