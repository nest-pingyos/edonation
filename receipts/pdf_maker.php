...
function fetchReceiptData($receiptId) {
    // API URL (internal call)
    $apiUrl = "http://localhost/appdev/edonation/api/v1/receipts/{$receiptId}/details";
...
if (!validateAccessToken($id, $token)) {
    ...
    <body>
        <div class="error-box">
            ...
            <a href="/appdev/edonation/receipts/">กลับไปหน้าค้นหา</a>
        </div>
    </body>
    </html>';
    exit;
...
function fetchSignatureConfig($year) {
    $apiUrl = "http://localhost/appdev/edonation/api/v1/signatures/{$year}";
